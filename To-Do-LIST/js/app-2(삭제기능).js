...
function todoRemoveHandler(e) {
  // 삭제 버튼을 정확하게 타켓하지 않는다면 함수 실행하지 않음
  if (!e.target.matches('.remove span')) return;
  console.log('삭제 클릭함!');

  // 클릭한 삭제 버튼을 감싸고 있는 li 태그를 제거해야 함
  //  1. 삭제버튼을 감사는 li 찾기
  const $targetLi = e.target.closest('.todo-list-item');
  console.log($targetLi);
  //  2. 해당 li를 !!!! 화면에서만 !!!! 삭제
  $targetLi.remove();
  //  3. 배열에서 해당 데이터를 날려야 함!!!!!!!!!!!!
  //  splice로 날리기 위해서는 인덱스를 찾아야 함 -> 태그에 들어있는 id 값을 찾아내야 함
  const dataId = $targetLi.dataset.id;
  console.log(`dataID: ${dataId}`);

  /* 1번째 방법 
  let index = todos.findIndex(todo => dataId === todo.id);
